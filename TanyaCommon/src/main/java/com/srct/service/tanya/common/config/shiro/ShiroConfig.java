/**
 * Title: ShiroConfig.java Description: Copyright: Copyright (c) 2019 Company: Sharp
 * 
 * @Project Name: TanyaCommon
 * @Package: com.srct.service.tanya.common.config.shiro
 * @author Sharp
 * @date 2019-02-05 21:38:40
 */
package com.srct.service.tanya.common.config.shiro;
...
import com.srct.service.tanya.common.config.shiro.filter.ShiroLogoutFilter;
import com.srct.service.tanya.common.config.shiro.utils.RedisManager;
import com.srct.service.tanya.common.config.shiro.utils.RetryLimitHashedCredentialsMatcher;
...
public class ShiroConfig {
    ...
    // /**
    // * 必须（thymeleaf页面使用shiro标签控制按钮是否显示） 未引入thymeleaf包，Caused by: java.lang.ClassNotFoundException:
     ...
     * 所以unauthorizedUrl设置后页面不跳转 Shiro注解模式下，登录失败与没有权限都是通过抛出异常。 并且默认并没有去处理或者捕获这些异常。在SpringMVC下需要配置捕获相应异常来通知用户信息
     * 
     * @return
     */
    @Bean
    ...
    public SimpleCookie rememberMeCookie() {
        ...
        simpleCookie.setMaxAge(3600 * 24 * 30);
        return simpleCookie;
    }
     ...
     */
    @Bean
    public ShiroRedisCacheManager shiroCacheManager() {
    ...
    /**
     * SessionDAO的作用是为Session提供CRUD并进行持久化的一个shiro组件 MemorySessionDAO 直接在内存中进行会话维护 EnterpriseCacheSessionDAO
    ...
    /**
     * 配置保存sessionId的cookie 注意：这里的cookie 不是上面的记住我 cookie 记住我需要一个cookie session管理 也需要自己的cookie 默认为: JSESSIONID 问题:
     * 与SERVLET容器名冲突,重新定义为sid
     * 
     * @return
     */
